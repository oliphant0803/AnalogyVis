...
// Define data domains
const rows = Array.from(new Set(data.map(d => d.grid))).sort(); // Sort to have 'A' at the top
...
const yScale = d3.scaleBand<string>()
  .domain(rows)
  .range([0, height]) // Reverse the range to have 'A' at the top
  .padding(0);

// Create custom color scale
const colorScale = d3.scaleSequential<number>()
  .domain([-24, 60])
  .interpolator(d3.interpolateRgbBasis(['darkblue', 'green', 'yellow', 'orange', 'red']));
...
const legendAxis = d3.axisLeft(legendScale)
  .tickValues(d3.range(60, -25, -6)) // Show every 6-degree interval
  .tickFormat(d => `${d}°C`);
